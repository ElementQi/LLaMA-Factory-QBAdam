...
from peft import DeltaConfig, DeltaModel
...
def _setup_delta_tuning(
    ...
    if is_trainable:
        logger.info("Fine-tuning method: {}".format("DoRA" if finetuning_args.use_dora else "Delta"))
    ...
    if model_args.adapter_name_or_path is not None:
        ...
        for adapter in adapter_to_merge:
            model: "DeltaModel" = PeftModel.from_pretrained(model, adapter, **init_kwargs)
            model = model.merge_and_unload()
    ...
    if is_trainable and adapter_to_resume is None:  # create new lora weights while training
        ...
        peft_kwargs = {
            "r": finetuning_args.lora_rank,
            "target_modules": target_modules,
            "delta_alpha": finetuning_args.lora_alpha,
            "delta_dropout": finetuning_args.lora_dropout,
            "use_rslora": finetuning_args.use_rslora,
            "modules_to_save": finetuning_args.additional_target,
        }
        ...
        if model_args.use_unsloth:
            ...
            model = get_peft_model(model, delta_config)
...
def init_adapter(
    ...
    """
    if is_trainable and getattr(model, "quantization_method", None) and finetuning_args.finetuning_type not in ["lora", "delta"]:

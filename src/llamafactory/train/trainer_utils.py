...
def _create_delta_optimizer(
    ...
    if finetuning_args.badam_mode == "layer":
        from peft.tuners.delta import BlockOptimizer
        # model.is_quantized = False
        ...
        logger.info(
            f"Using Delta optimizer with layer-wise update, switch mode is {finetuning_args.badam_switch_mode}, "
            f"switch block every {finetuning_args.badam_switch_interval} steps, "
            f"default start block is {finetuning_args.badam_start_block}"
        )
...
def create_custom_optimzer(
    ...
    if finetuning_args.use_delta:
        return _create_delta_optimizer(model, training_args, finetuning_args)
